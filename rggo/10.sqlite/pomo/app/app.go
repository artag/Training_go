...
// Instatiate the new application
func New(config *pomodoro.IntervalConfig) (*App, error) {
	...
	controller, err := termdash.NewController(
		term, c, termdash.KeyboardSubscriber(quitter))
	if err != nil {
		return nil, err
	}
...
// Resize the interface if needed
...
// Run and control the application
func (a *App) Run() error {
	defer a.term.Close()
	defer a.controller.Close()

	// To check if a resize is needed
