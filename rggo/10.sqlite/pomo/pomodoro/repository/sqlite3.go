...
func (r *dbRepo) Create(i pomodoro.Interval) (int64, error) {
	...
	insStmt, err := r.db.Prepare(
		"INSERT INTO interval VALUES(NULL, ?, ?, ?, ?, ?)")
	if err != nil {
		return 0, err
	}
	defer insStmt.Close()
...
func (r *dbRepo) Update(i pomodoro.Interval) error {
	...
	res, err := updStmt.Exec(
		i.StartTime, i.ActualDuration, i.State, i.ID)
	if err != nil {
		return err
	}
...
func (r *dbRepo) ByID(id int64) (pomodoro.Interval, error) {
	...
	i := pomodoro.Interval{}
	err := row.Scan(
		&i.ID, &i.StartTime, &i.PlannedDuration, &i.ActualDuration, &i.Category, &i.State)
	return i, err
}
...
func (r *dbRepo) Breaks(n int) ([]pomodoro.Interval, error) {
	...
	for rows.Next() {
		i := pomodoro.Interval{}
		err = rows.Scan(
			&i.ID, &i.StartTime, &i.PlannedDuration,
